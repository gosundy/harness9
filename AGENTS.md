...
| `github.com/larksuite/oapi-sdk-go/v3` | v3.6.1 | 飞书（Lark）官方 Go SDK（WebSocket 长连接 + IM API） |
...
- `github.com/gorilla/websocket` — WebSocket 通信（飞书 SDK 依赖）
...
│   ├── imchannel/                   # IM 平台统一适配层
│   │   ├── channel.go               # IMChannel / Session 接口定义（平台无关契约）
│   │   └── feishu/                  # 飞书（Lark）平台实现
│   │       ├── client.go            # 飞书 WebSocket 长连接适配器（IMChannel 实现）
│   │       ├── session.go           # 飞书 Session 实现（思考/工具进度/最终回复）
│   │       └── session_test.go      # 辅助函数单元测试（truncateRunes / buildTextContent）
...
└──────────┬──────────────────────┬───────────────┘
           │                      │
┌──────────▼──────────┐  ┌────────▼────────────────┐
│    engine (编排层)    │  │  imchannel (IM 适配层)    │
│  Run / RunStream     │  │  IMChannel 接口           │
│  Two-Stage ReAct     │  │  feishu/Channel（WebSocket│
│  工具调度 / 终止检测   │  │  feishu/Session（进度推送）│
└────┬────────┬────────┘  └─────────────────────────┘
     │        │
┌────▼──┐  ┌──▼────────┐  ┌───────────┐
│provid │  │  schema   │  │  tools    │
...
> **Roadmap（暂未启动）**：会话记忆持久化（memory）、Slack / 钉钉等其他 IM 渠道接入。
...
# 启动飞书 Bot（需在 .env 中配置 FEISHU_APP_ID / FEISHU_APP_SECRET / OPENAI_API_KEY）
go run ./cmd/harness9
```

> `engine.Run`（阻塞模式）和 `engine.RunStream`（流式模式）仍作为内部 API 可用，
> 飞书 Bot 使用 RunStream 驱动进度推送，如需 CLI 模式可直接调用 engine.Run。
...
#### IMChannel / Session 接口契约
- `IMChannel.SetMessageHandler` 必须在 `Start` 之前调用；处理器不应长时间阻塞，耗时操作放入独立 goroutine
- `Server.handleMessage` 通过从 server ctx 派生的独立子 context 处理每条消息，超时为 5 分钟
- `Session.SendReply` 调用方保证 text 非空；若 agent 静默完成，Server 编排层（`server.go`）负责提供兜底文本（`"✅ 任务完成"`）

#### 飞书进度消息策略
- 进度展示采用"**独立消息**"模式，而非 Patch API。飞书 Patch API 仅支持交互式卡片（`msg_type=interactive`），不适用于纯文本消息
- 消息序列：`🤔 思考中...` → `💭 思考内容摘要` → `🔧 调用工具：xxx` × N → `✅/❌ 工具名（耗时ms）` × N → 最终回复
- Thinking 摘要截断阈值：`maxThinkingRunes = 400` Unicode 字符
- 飞书消息并发安全：`Session` 本身无可变状态，多 goroutine 并发调用不同方法是安全的

#### Server 事件映射规则
| engine.Event | Server 行为 |
|---|---|
| `EventThinkingDelta` | 累积到 `lastThinking`，Turn 变化时重置 |
| `EventActionDelta` | 累积到 `reply` |
| `EventToolStart` | flushThinking()，重置 reply，记录工具元信息，调用 `NotifyToolStart` |
| `EventToolResult` | 查询工具元信息，计算耗时，调用 `NotifyToolDone` |
| `EventDone` | reply 非空 → flushThinking + SendReply；reply 为空 → 用 thinking/兜底文本 SendReply |
| `EventError` | SendReply("❌ " + errMsg) |

### 6.8 第三方 API / SDK 使用规范
...
- `github.com/larksuite/oapi-sdk-go/v3` — 飞书官方 Go SDK（WebSocket 长连接 + IM 消息 API）
