...
│  标准 ReAct          │  │  feishu/Channel（WebSocket│
...
- 消息序列：`🤔 思考中...` → `🔧 调用工具：xxx` × N → `✅/❌ 工具名（耗时ms）` × N → 最终回复
...
| `EventToolStart` | 重置 `reply`，记录工具元信息，调用 `NotifyToolStart` |
| `EventToolResult` | 查询工具元信息，计算耗时，调用 `NotifyToolDone` |
| `EventDone` | reply 非空 → SendReply(reply)；reply 为空 → SendReply("✅ 任务完成") |
