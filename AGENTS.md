...
│   │   ├── todo_write.go            # todo_write 工具（读写 TodoStore + 状态转换校验）
...
| 模块 | 职责 | 状态 |
|------|------|:----:|
| **cmd/harness9** | 主入口：TTY 自动检测选择 TUI / CLI；初始化 Memory Manager + SummarizationCompactor + 首个 Session | ✅ |
| **tui** | 全屏 TUI（Bubbletea）：WelcomeBanner + 对话页双 Phase、Spinner 动词轮换、内置命令 Tab 补全 + Skills 补全、Markdown 渲染、会话管理、状态栏 token 用量实时展示（颜色告警）+ 压缩通知 + Plan Mode 色调 + 审查对话框 + autoExecuting 续跑 | ✅ |
| **engine** | 标准 ReAct 主循环，阻塞 + 流式双模式，并发工具调度，Session/Compactor 集成，EventTokenUpdate / EventCompaction 事件，WithContextWindow 选项，PlanMode 工具过滤 + prompt 注入，TodoStore 跨会话持久化 | ✅ |
| **planning** | PlanMode 枚举（Default/Plan/AutoEdit）、TodoStore（线程安全，全量替换）、TodoItem 状态机、todo_write 工具（状态转换校验：pending→completed 被拒绝） | ✅ |
| **memory** | Context Engineering：Session 接口、Manager（SQLite CRUD）、SQLiteSession（WAL + 事务）、SummarizationCompactor（默认，LLM 摘要压缩 + 增量更新 + 错误回退）、TokenBudgetCompactor（回退，80% 阈值 + 孤立工具对双向修复）、SlidingWindowCompactor（回退方案）、token 估算工具 | ✅ |
| **provider** | LLM 统一接口 + OpenAI / Anthropic SDK 适配器 + 实际 token 用量提取（Usage 类型）+ 模型 context window 注册表 | ✅ |
| **schema** | 跨组件共享的核心数据类型（Message、ToolCall、Usage 等） | ✅ |
| **tools** | 工具注册表 + 内置工具（bash / read_file / write_file / edit_file / todo_write）+ 路径沙箱 | ✅ |
