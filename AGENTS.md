...
- **MCP 工具集成（Model Context Protocol）**: `internal/mcp/` 包；JSON-RPC 2.0 over stdio/HTTP；`Config`（`.mcp.json` 加载，file-not-found 静默返回空）+ `StdioTransport`（subprocess + NDJSON async reader goroutine + pending map ID 关联 + 三路 select ctx/done/response）+ `HTTPTransport`（无状态 POST）+ `Client`（initialize → notifications/initialized → tools/list → tools/call 握手与调用）+ `Manager`（并发 Start 30s per-server timeout，fail-soft，`ServerStatus` + `ToolDetails` TUI 通知链，`InjectTools` 无循环变量 bug 闭包，`WithNotify` channel 回调）+ `MCPToolAdapter`（实现 `BaseTool` 接口，`mcp__{server}__{tool}` 双下划线命名，对 Engine 完全透明）；TUI MCPBar（状态栏实时展示）+ `/mcp` 模态工具面板（工具列表，`e` 键 `tea.ExecProcess` 打开 `$EDITOR` 编辑 `.mcp.json`）；`.mcp.json` 配置文件驱动，main.go 中异步启动不阻塞 TUI 渲染；`defer mcpMgr.Stop()` Session 级生命周期
...
│   │   ├── transport.go             # Transport 接口 + StdioTransport（subprocess + async reader goroutine）+ HTTPTransport
│   │   ├── client.go                # Client（initialize → notifications/initialized → tools/list → tools/call 握手与调用）
│   │   ├── manager.go               # Manager（并发连接、ServerStatus + ToolDetails、InjectTools、WithNotify）
│   │   └── manager_test.go          # Manager + mockTransport 单元测试
...
│   ├── autodev/
│   │   └── SKILL.md                 # /autodev AgentSkill：自举开发三阶段工作流
│   ├── debugging-guide/
│   │   └── SKILL.md                 # Go 调试指南
│   ├── go-coding-standards/
│   │   └── SKILL.md                 # Go 编码规范
